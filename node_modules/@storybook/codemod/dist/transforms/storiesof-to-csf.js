...
function transformer(file, api, options) {
  ...
  function extractDecorators(parameters) {
    ...
    const storyParams = _objectSpread({}, parameters);
  ...
  function convertToModuleExports(path, originalExports) {
    const base = j(path);
    const statements = [];
    const extraExports = []; // .addDecorator

    const decorators = [];
    base.find(j.CallExpression).filter(call => call.node.callee.property && call.node.callee.property.name === 'addDecorator').forEach(add => {
    ...
    const parameters = [];
    base.find(j.CallExpression).filter(call => call.node.callee.property && call.node.callee.property.name === 'addParameters').forEach(add => {
    ...
    if (originalExports.length > 0) {
      extraExports.push(j.property('init', j.identifier('excludeStories'), j.arrayExpression(originalExports.map(exp => j.literal(exp)))));
    } // storiesOf(...)


    base.find(j.CallExpression).filter(call => call.node.callee.name === 'storiesOf').filter(call => call.node.arguments.length > 0 && call.node.arguments[0].type === 'Literal').forEach(storiesOf => {
    ...
    const adds = [];
    base.find(j.CallExpression).filter(add => add.node.callee.property && add.node.callee.property.name === 'add').filter(add => add.node.arguments.length >= 2 && add.node.arguments[0].type === 'Literal').forEach(add => adds.push(add));
    adds.reverse();
    adds.push(path);
    const identifiers = new Set();
    root.find(j.Identifier).forEach(({
      value
    }) => identifiers.add(value.name));
    adds.forEach(add => {
      let name = add.node.arguments[0].value;
      const sanitized = (0, _utils.sanitizeName)(name);
      let key = sanitized;
      let counter = 0;
      ...
      if ((0, _router.storyNameFromExport)(key) === name) {
        name = null;
      }
      ...
      if (add.node.arguments.length > 2) {
        ...
        const {
          storyParams,
          storyDecorators
        } = extractDecorators(originalStoryParams);
  ...
  const initialStoriesOf = root.find(j.CallExpression).filter(call => call.node.callee.name === 'storiesOf');
  const defaultExports = root.find(j.ExportDefaultDeclaration); // If there's already a default export
  ...
  root.find(j.ExportNamedDeclaration).forEach(exp => originalExports.push(exp.node.declaration.declarations[0].id.name)); // each top-level add expression corresponds to the last "add" of the chain.
  // replace it with the entire export statements

  root.find(j.CallExpression).filter(add => add.node.callee.property && add.node.callee.property.name === 'add').filter(add => add.node.arguments.length >= 2 && add.node.arguments[0].type === 'Literal').filter(add => add.parentPath.node.type === 'ExpressionStatement').forEach(path => convertToModuleExports(path, originalExports)); // remove storiesOf import

  root.find(j.ImportSpecifier).filter(spec => spec.node.imported.name === 'storiesOf' && spec.parent.node.source.value.startsWith('@storybook/')).forEach(spec => {
